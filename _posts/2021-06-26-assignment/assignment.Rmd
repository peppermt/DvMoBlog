...
packages = c('DT','tidyverse', 'ggiraph', 'plotly', 'lubridate', 'naniar','stringr','dplyr','ggstance','ggpubr','hms','data.table','raster','sf','tmap','clock','rgdal','tibbletime','networkD3', 'mapview','scales')
...
```{r special char in katerinas cafe, echo=FALSE}
...
```{r importing vector GIS data file}

Abila_st <- st_read(dsn = "data/MC2/Geospatial",
                    layer = "Abila")

```
Then, we shall map the aspatial data next. This gps file had already been imported earlier as _gpstracking_. We would need to convert it into a Simple Feature Data Frame using _st_as_sf()_ of the _sf_ package. Coordinates would no longer be in dbl format, but would be in geometry format. They would be input as longitude ('long' ; x-coordinates) and latitude ('lat' ; y-coordinates), in the EPSG: 4326 format, which is the wgs84 Geographic Coordinate System.

```{r Conversion into Simple Feature Data Frame}
...
```{r Creating Movement Path, include=FALSE}
...
```{r creating date and time versions for sf, message=FALSE}
...
```{r code to select car to show gps path, echo=TRUE}
...
```{r code to select car and time to show gps path, echo = FALSE}
...
gpstracking$thirtymins <- hms::as_hms(round_date(gpstracking$TimeStampFormatted, "30 mins"))


gps_pts_selected <- gpstracking %>% 
  filter(time >= hms::as.hms("22:20:00"))

pts_sf <- st_as_sf(gps_pts_selected,
                   coords = c("long","lat"),
                   crs = 4326)
...
```{r for time of the day, echo = FALSE, layout="1-body-outset", fig.width=16, fig.height=10}
...
```{r trying out geom_tile, eval=FALSE, fig.height=10, fig.width=16, include=FALSE, layout="1-body-outset"}
...
```{r same plot by day via loyalty card, eval=FALSE, include=FALSE}

x3 <- length(unique(loyalcardbyday$n))
cc_colours3 <- colorRampPalette(c('green', 'yellow', 'orange', 'red'))(x3)
...
```{r Visits at each Location by Time, eval=FALSE, include=FALSE}

#YlGnBu

x <- length(unique(cardbytime$n))
cc_colours3 <- colorRampPalette(c('green', 'yellow', 'orange', 'red'))(x3)
...
From _Visits at Each Location for Each Day_, we could see that Katerina's Cafe, Hippokampos, Guy's Gyros and Brew've Been Served have the lightest bars across the 2 weeks. This shows that they are consistently popular. We also note that Guy's Gyros is more popular during weekdays than weekends, and Brew've Been Served only has visits on weekdays, indicating that it is probably not open on weekends.

No. | Location | Periods Popular | Remarks
----| ---------| ----------------| -------
1 | Katerina's Cafe | All Days ; 1300 hrs - 1430 hrs, 1900 hrs - 2130 hrs | NIL
2 | Hippokampos | All Days ; 1230 hrs - 1430 hrs, 1930 hrs - 2230 hrs | NIL
3 | Guy's Gyros | More Popular During Weekdays ; 1300 hrs - 1430 hrs, 1900 hrs - 2130 hrs | Unusually crowded on 19 Jan (Sun) compared to the previous Sunday and weekend crowd
4 | Brew've Been Served | Weekday Mornings ; 0730 hrs - 0830 hrs | Huge Crowd at 0800 hrs ; Seems closed on weekends
...
![](images/Top3LocationsVisited.png)
...
```{r time periods of travel of each personnel, eval=FALSE, include=FALSE}
...
We also notice that there is a high count of travels on weekdays with LastName = NA. Checking on the _gpsname_ data, we find that there is gps tracking information for vehicle ID 105 and 107, which do not exist in the _car_assigned_ dataset, this is why LastName was NA. Tracking of the route for vehicle ID 105 and 107 are as below.
...
```{r car_assigned missing values show truck drivers}
...
```{r each personnel travel time period on each day, eval=FALSE, fig.height=10, fig.width=12, include=FALSE, layout="1-page"}
