...
#Identifying stops, filtering stop coordinates by Car ID and Time Period

stops <- gpstracking %>% as_tbl_time(index=TimeStampFormatted) %>%
  group_by(id) %>%
  filter(id == 15) %>% 
  filter_time('2014-01-06 07:00:00'~'2014-01-06 09:00:00') %>% 
...
#Filtering stop coordinates by car ID
...
With the above codes, the following function is created to generate the routes of a specific vehicle, during a specific time period.
...
print_routes_ID_date <- function(emp_id,datetime){
...
This means that there are transactions paid by cash, not paid by credit card. There were also transactions made without using loyalty card. But business still seems abnormally bad.
