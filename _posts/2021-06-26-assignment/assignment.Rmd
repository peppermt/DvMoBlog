...
gps_indivA <- gpsnamegender %>% filter(name %in% onetosix$name)  %>% 
                    ...
                    distinct(name,date,time,namecountperday, Gender)

indivA <- ggplot(gps_indivA, aes(time, date, fill=Gender, group=name)) +
...
```{r for the other individual groups 2 to 6}
...
#OBSOLETED
testfigF <- gpsname %>% filter(name == "Willem Vasco-Pais") %>% 
  group_by(name) %>% mutate(namecountperday = n()) %>% 
  ...
  scale_y_date(breaks = breaks_pretty(14)) +
  scale_x_datetime(breaks = breaks_pretty(24)) +
  ggtitle("Travel Periods for") + 
  labs(x = "Time")

chartviz <- ggplotly(testfigF)
