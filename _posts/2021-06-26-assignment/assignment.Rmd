...
```{r Creating Movement Path, echo=TRUE, results=}
...
```{r trying out geom_tile, echo = FALSE, layout="1-body-outset", fig.width=16, fig.height=10}
...
```{r same plot by day via loyalty card, echo=FALSE}

figE <- loyalcardbydayA %>% 
...
```{r Visits at each Location by Time, echo = FALSE}

#YlGnBu

x3 <- length(unique(cardbytime$n))
...
```{r code to plot popular gps points CAN REMOVE THIS CHUNK}
...
```{r time periods of travel of each personnel}
...
```{r each personnel travel time period on each day, layout="1-page", fig.width=12, fig.height=10}
