...
We shall use the violin plots in the _plotly_ package as they are interactive, we can hover over the plots, especially where they are thicker to see what time periods are those. There are also boxplots to tell us what the distribution and spread is like. 
...
figA <- cardbyday %>% 
 ggplot(aes(date, location)) +
  ...
  scale_x_datetime(breaks = breaks_pretty(14)) +
...
# 5.1.1 Anomalies Detected

## 5.1.1.1 Hippokampos is nowhere to be found on the map of Abila 
...
## 5.1.1.2 Awfully infrequent visits at refuel stations U Pump and Frank's Fuel
...
figI <- cardbyday %>% filter(location %in% c("U Pump", "Frank s Fuel")) %>% 
 ggplot(aes(date, location)) +
  geom_tile(aes(fill = as.factor(n))) +
      labs(title = "Visits at each Fuel Station for Each Day (Credit Card)", x = "Date", fill = "Visits") +
  scale_x_datetime(breaks = breaks_pretty(14)) +
    theme(axis.text.x = element_text(angle = 45)) +
  theme(legend.key.height = unit(1, "cm"))
...
figK <- loyalcardbyday %>% filter(location %in% c("U Pump", "Frank s Fuel")) %>%
 ggplot(aes(TimeStampFormatted, location)) +
  geom_tile(aes(fill = as.factor(n))) +
    labs(title = "Visits at each Fuel Station for Each Day (by Loyalty Card)", x = "Date", fill = "Visits") +
  scale_x_datetime(breaks = breaks_pretty(14)) +
    theme(axis.text.x = element_text(angle = 45)) +
  theme(legend.key.height = unit(1, "cm"))
...
## 5.1.1.3 Unusually infrequent visits for supermarkets
...
figC <- cardbyday %>% filter(location %in% c("Kronos Mart", "General Grocer")) %>% 
 ggplot(aes(date, location)) +
  geom_tile(aes(fill = as.factor(n))) +
      labs(title = "Visits at each Supermarket for Each Day (Credit Card)", x = "Date", fill = "Visits") +
  scale_x_datetime(breaks = breaks_pretty(14)) +
    theme(axis.text.x = element_text(angle = 45)) +
  theme(legend.key.height = unit(1, "cm"))
...
figD <- loyalcardbyday %>% filter(location %in% c("Kronos Mart", "General Grocer")) %>%
 ggplot(aes(TimeStampFormatted, location)) +
  geom_tile(aes(fill = as.factor(n))) +
    labs(title = "Visits at each Supermarket for Each Day (by Loyalty Card)", x = "Date", fill = "Visits") +
    theme(legend.key.height = unit(1, "cm")) +
    scale_x_datetime(breaks = breaks_pretty(14)) +
    theme(axis.text.x = element_text(angle = 45))
...
## 5.1.1.4 High Transaction Amounts
...
```{r tabulate transaction amount outliers, echo=FALSE, layout = "1-body-outset"}
...
rmarkdown::paged_table(txn_outliertab)    
...
### 5.2.1 Anomaly: Hippokampos nowhere to be found on Abila Map

We need to be able to find some points which distinguishes Hippokampos from the other locations. I noted from the _Visits at each Location for Time of Day_ plot that only Hippokampos had a transaction happening at around 2230 hrs. 

![](Images/hippotxntime.jpg)
Hence, we shall filter the gps data by this time period, to see where the possible stops are at.

```{r scatter plot of hippokampos, include=FALSE}
...
### 5.2.2 Anomaly: Unusually Infrequent Visits at Refuel Stations and Supermarkets 
...
```{r gps dataset wrangling, include=FALSE}
...
```{r each personnel travel time period on each day, eval=FALSE, include=TRUE, fig.height=10, fig.width=12}

#Do a left join to add gender to gpsname

gpsnamegender <- left_join(gpsname, emprecords, by = c("LastName" = "LastName", "FirstName" = "FirstName"))

gpsnamegender <- gpsnamegender %>% dplyr::select(c("TimeStampFormatted","name","id","lat","long","time","hour","date","Gender","CurrentEmploymentTitle.x","CurrentEmploymentType.x","LastName","FirstName"))

#Dot Plot of Day with Time for Each Individual

namedistinct <- gpsname %>% 
    distinct(name)

#Slicing the namelist into individuals
onetosix <- namedistinct %>% slice(1:6)
sevtotwelve <- namedistinct %>% slice(7:12)
thirteentoeighteen <- namedistinct %>% slice(13:18)
nineteentotwofour <- namedistinct %>% slice(19:24)
twofivetothirty <- namedistinct %>% slice(25:30)
thirtyonetothirtysix <- namedistinct %>% slice(31:36)

#Plotting the chart
gps_indivA <- gpsnamegender %>% filter(name %in% onetosix$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivA <- ggplot(gps_indivA, aes(time, date, fill=Gender, label = lat, label2 = long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point() +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 1)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))
...
![](Images/IndivPlot.png)


```{r plotting the individual charts for the other groups, eval=FALSE, fig.height=10, fig.width=12, include=FALSE}

#Plotting the chart B
gps_indivPB <- gpsnamegender %>% filter(name %in% sevtotwelve$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivPB <- ggplot(gps_indivPB, aes(label=lat, label2=long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point(aes(time, date, fill=Gender)) +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 2)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))

ggplotly(indivPB)

#Plotting the chart C
gps_indivPC <- gpsnamegender %>% filter(name %in% thirteentoeighteen$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivPC <- ggplot(gps_indivPC, aes(label=lat, label2=long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point(aes(time, date, fill=Gender)) +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 3)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))

ggplotly(indivPC)

#Plotting the chart D
gps_indivPD <- gpsnamegender %>% filter(name %in% nineteentotwofour$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivPD <- ggplot(gps_indivPD, aes(label=lat, label2=long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point(aes(time, date, fill=Gender)) +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 4)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))

ggplotly(indivPD)

#Plotting the chart E
gps_indivPE <- gpsnamegender %>% filter(name %in% twofivetothirty$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivPE <- ggplot(gps_indivPE, aes(label=lat, label2=long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point(aes(time, date, fill=Gender)) +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 5)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))

ggplotly(indivPE)

#Plotting the chart F
gps_indivPF <- gpsnamegender %>% filter(name %in% thirtyonetothirtysix$name)  %>% 
                    group_by(name) %>% mutate(namecountperday = n()) %>% 
                    distinct(name,date,time,namecountperday, Gender, lat, long)

indivPF <- ggplot(gps_indivPF, aes(label=lat, label2=long)) +
                facet_wrap(.~name, nrow = 18, ncol = 2) +
                geom_point(aes(time, date, fill=Gender)) +
                scale_y_date(breaks = breaks_pretty(14)) +
                labs(title = "Travel Periods of Individuals (Group 6)", x = "Time") +
                theme(axis.text.x = element_text(angle = 45))
...
From observation of the above _Travel Periods of Individuals_ plots, matched with their daily routes filtered by specific periods, I managed to nail down the coordinates of the following locations:
...
### 5.3.1 From Movement of CEO Sten Sanjorge Jr.
...
```{r Chostus Hotel Transactions, echo=FALSE}

pgtable <- creditcard %>% filter(location == "Chostus Hotel")

rmarkdown::paged_table(pgtable)

```
### 5.3.2 To Match Credit Card and Loyalty Card

```{r Look at CC and Loyalty Card Matched, echo=FALSE, fig.height=10, fig.width=12}
...
_( ) indicates number of transactions_
...
2) One credit card associated with multiple loyalty cards:
...
* 5921 associated with L3295 (12) and L9406 (1) [ _Opposite pattern with 4948_ ]
* 7889 associated with L2247 (1) and L6119 (20) [ _Opposite pattern with 5368_ ]
* 8332 associated with L2070 (27) and L8566 (1) [ _Opposite pattern with 4795_ ]

The following conclusions can be drawn:

* L3288 is associated with both 1286 and 9241 at almost equal transactions. This can show that L3288 belongs to either owners of 1286 or 9241, and that they are often together when these transactions are made. They might be a couple. 13 to 15 transactions within a 2-week period is almost daily transaction.

* Owner of 4948 was with owner of 5921 when they used each other's loyalty cards and paid with their own credit card on 2 occasions. They could be friends.

* Owner of 5368 was with owner of 7889 when they used each other's loyalty cards and paid with their own credit card on 2 occasions. They could be friends.

* Owner of 4795 was with owner of 8332 when they used each other's loyalty cards and paid with their own credit card on 2 occasions. They could be friends.
...
### 5.4.2 Potential Unofficial Relationship: Brand Tempestad and Elsa Orilla
...
Their movements from morning, till the time they returned to GasTech after their stop at Chostus Hotel, for both 8th and 14th Jan are as follows. These timings have been extracted from the plots of _Travel Periods of Individuals_, and their stops deduced from the route and stops generated by our coded function *print_routes_ID_date()*. We can see from the images below that their travel behavior is similar.

Name | Movements
-----| ---------
Brand Tempestad <br> Car ID: 33 | **08 Jan** <br> 0712 hrs: Set off from home <br> 0754 hrs: Reached GasTech <br> 1102 hrs: Left GasTech <br> 1109 hrs: Reached Chostus Hotel <br> 1321 hrs: Left Chostus Hotel <br> 1338 hrs: Returned to GasTech <br> <br> **14 Jan** <br> 0724 hrs: Set off from home <br> 0748 hrs: Reached GasTech <br> 1101 hrs: Left GasTech <br> 1108 hrs: Reached Chostus Hotel <br> 1332 hrs: Left Chostus Hotel <br> 1339 hrs: Returned to GasTech
Elsa Orilla <br> Car ID: 7 | **08 Jan** <br> 0720 hrs: Set off from home <br> 0724 hrs: Stopped near Coffee Shack <br> 0753 hrs: Left Coffee Shack <br> 0806 hrs: Reached GasTech <br> 1110 hrs: Left GasTech <br> 1117 hrs: Reached Chostus Hotel <br> 1301 hrs: Left Chostus Hotel <br> 1308 hrs: Returned to GasTech <br> <br> **14 Jan** <br> 0721 hrs: Set off from home <br> 0724 hrs: Reached Coffee Shack <br> 0750 hrs: Left Coffee Shack <br> 0803 hrs: Reached GasTech <br> 1113 hrs: Left GasTech <br> 1120 hrs: Reached Chostus Hotel <br> 1320 hrs: Left Chostus Hotel <br> 1327 hrs: Returned to GasTech

![](images/BrandTempestadCouple.jpg)

![](images/ElsaCouple.jpg)
...
### 5.5.1 Sten Sanjorge Jr. only had 3 day of GPS records from 17th to 19th Jan, while everyone else travelled almost daily.
```{r time periods of travel of each personnel, echo=FALSE, fig.height=10, fig.width=12}
...
figC <- dgpsname %>% 
  ...
  scale_x_date(breaks = breaks_pretty(14)) +
  theme(axis.text.x = element_text(angle = 45))
...
```{r Chostus Hotel Transactions 2, echo=FALSE}

pgtblA <- creditcard %>% filter(location == "Chostus Hotel")

rmarkdown::paged_table(pgtblA)

```
![](images/CEOmovement.jpg)

### 5.5.2 Unusually high movement activity for truck on 16 Jan 2014.
...
truckers <- car_assigned %>% 
  filter(is.na(CarID))

rmarkdown::paged_table(truckers)
...
* Vehicle 106, 15 Jan, covered much smaller route than usual
* Vehicle 107, 15 Jan, made extra stops at Maximum Iron & Steel, an unknown location westwards of Maximum Iron & Steel, and Carlyle Chemical Inc.
...
### 5.5.3 Supermarket Transactions in Wee Hours, with High Transaction Amounts
...
figH <- cardbytime %>% filter(location %in% c("Kronos Mart", "General Grocer")) %>%
 ggplot(aes(thirtymins, location)) +
  geom_tile(aes(fill = as.factor(n))) +
      labs(title = "Visits at each Location for Time of Day", x = "Time", fill = "Visits") +
    scale_x_datetime(breaks = date_breaks("1 hour"),
                   labels = time_format("%H:%M")) +
  theme(axis.text.x = element_text(angle = 45))
...
```{r midnight transactions at Kronos Mart, echo=FALSE}

kronos <- creditcard %>% filter(location == "Kronos Mart") %>% 
  arrange(time)  %>%  slice_min(time,n = 6)

rmarkdown::paged_table(kronos)

```

### 5.5.4 Other Anomalies to be Investigated
...
* High Transaction Amounts across 2 weeks, as specified in Q1 anomalies detected
